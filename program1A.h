...
#define esLetra(x)             (logico)(strchr("ABCDEFGHIJKLMNÑOPQRSTUVWXYZabcdefghijklmnñopqrstuvwxyzáéíóúüÁÉÍÓÚÜ",x))
#define esSigno(x)             (NO (esLetra(x) O esDigito(x)))
#define esUconDieresis(x)      (logico)(strchr("üÜ",x))
#define esVocal(x)             (logico)(strchr("AEIOUaeiouáéíóúüÁÉÍÓÚÜ",x))
#define esVocalAcentuada(x)    (logico)(strchr("áéíóúÁÉÍÓÚ",x))
#define esVocalAbierta(x)      (logico)(strchr("AEOaeoáéóÁÉÓ",x))
#define esVocalCerrada(x)      (logico)(strchr("IUiuíúüÍÚÜ",x))
#define esDiptongo(x,y)        (logico)(esVocal(x) Y esVocal(y) Y NEQV(esVocalAbierta(x),esVocalAbierta(y)))
#define esTriptongo(x,y,z)     (logico)(esVocalCerrada(x) Y esVocalAbierta(y) Y esVocalCerrada(z))
#define esHiato(x,y)           (logico)(esVocalAbierta(x) Y esVocalAbierta(y))
...
inline entero posCarEnCad(caracter car,cadena cad) {regresa(cad.find(car));}
...
inline funcion logico noTieneDieresis(noCambia cadena porRef cad) {
